...
## What it does
Our project uses a specialized, easy-to-use Chrome Extension to bridge the gap between complex legal documents and user understanding. By feeding the active page's content into the Google Gemini API, the tool performs a real-time audit of the agreement. It returns a numeric risk and privacy score alongside a breakdown of high-priority concerns and specific rights the user is asked to waive.
...
2. Content Extraction (Document Object Model API/ pdf.js lib): When triggered, a Content Script uses the DOM API and pdf.js to scrape the active website's Terms & Conditions text, ensuring we capture the exact legal language the user is agreeing to. 
...
4. Data Pre-processing(Regex patterns): A collection of regex patterns are used to clean up an extrenous data in the TOS and extract specific patterns that the user is concerned about. 
5. AI Analysis (Google Gemini API): The Flask app constructs a specialized prompt containing the T&C text and sends it to Google Gemini. The model is instructed to perform a legal audit, specifically looking for privacy risks and rights-forfeiture.
6. Data Feedback Loop: Gemini returns a structured analysis, which the Flask app parses and sends back to the Chrome Extension to display the final Numeric Risk Score and key feedback to the user.
...
Deploying our flask app was challenging as we had to connect the port that the WSGI server was listenting on. Desptie this, we were able to work together to debug that the issue was coming from mislligned ports. 


## Accomplishments that we're proud of
1. Boost how well the risk factor alligns with the user concerns by pre-processing terms and conditions as adding specific parameters for the model to look for in specific high risk terms.
2. Additionally we have successfully deployed our Chrome extension through Railway and no longer need to rely on a local host to run the extension and create the API calls.
3. Allow users to have safety preferences and allow them to highlight and mark off the risks they care the most about to allow the Gemini API to dive deeper in the specified risks.
4. Devlopment of Easy-to-use UI, which is one of the main reasons we chose to build a chrome extension instead of a webstie. 
